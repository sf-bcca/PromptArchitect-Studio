...
Model Context Protocol (MCP) servers extend Antigravity's capabilities by providing specialized tools for various domains.
...
2. **Environment**: Docker ensures consistency across different development setups.
3. **Backend Integration**: Supabase MCP allows for direct interaction with our database and edge functions without leaving the development environment.
4. **Validation**: Automated tasks and walkthroughs ensure that every change is verified and documented.
